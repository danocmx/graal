...
public class RematerializationHandler {
    ...
    ConstantMaterializationConflictResolver constantMaterializationConflictResolver;

    protected RematerializationHandler(ConstantMaterializationConflictResolver constantMaterializationConflictResolver) {
    ...
    /**
     * Find operands pre allocation for rematerialized instruction.
     *
     * 0. Collect all instructions before allocation 1. Copy current locations to a new Op instance
     * 2. Check that all location states previously copied have a state (not unknown) 3. Get all ops
     * based on LIRInstruction class of said instruction 4. Go over every op, check if operand
     * counts match 5. Check if verifier state matches the original variable 6. If that is the case
     * for every operand, we found rematerialized an instruction
     *
    ...
    protected boolean doLocationStatesMatchOriginals(RAVInstruction.ValueArrayPair locations, RAVInstruction.ValueArrayPair originals, BlockVerifierState blockState) {
        for (int i = 0; i < locations.count; i++) {
            ...
            // TODO: solve conflict resolvers at this place!
            return false;
        }
